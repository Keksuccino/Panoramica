...
public class PanoramicaHandler {
	...
	private void takeScreenshot() {
		this.screenshots.add(Screenshot.takeScreenshot(mc.getWindow().getWidth(), mc.getWindow().getHeight(), mc.getMainRenderTarget()));
	}
	...
	private boolean finishPanorama() {
		try {
			if (this.screenshots.size() == 6) {
				...
				TranslatableComponent compSuccessMsg = new TranslatableComponent("screenshot.success", compSaveDir);
